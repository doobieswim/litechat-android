...
def main() -> int:
    ...
    ok("stop imagine job", "streamJob = viewModelScope.launch" in vm.split('// C-011: /imagine slash command')[1][:800] if '// C-011: /imagine slash command' in vm else False)
    ok("stop video job", "streamJob = viewModelScope.launch" in vm.split('// C-027: /video command')[1][:800] if '// C-027: /video command' in vm else False)
    ok("stop browse job", "streamJob = viewModelScope.launch" in vm.split('// C-013: /browse command')[1][:800] if '// C-013: /browse command' in vm else False)
